...
熵（entropy）：
熵指的是体系的混乱的程度，在不同的学科中也有引申出的更为具体的定义，是各领域十分重要的参量,越混乱，熵值越大。
...
在划分数据集前后信息发生的变化称为信息增益，即不知道任何特征时的分类结果的信息熵减去在某一特征分类下的条件熵。

 条件熵：H(Y|X)，当X条件下Y的信息熵 ，其计算公式为：   

![条件熵的计算公式](/img\ml\3.DecisionTree\条件熵计算公式.jpg)    
 信息增益： IG(Y|X)=H(Y)-H(Y|X)条件熵的计算公式如下：
